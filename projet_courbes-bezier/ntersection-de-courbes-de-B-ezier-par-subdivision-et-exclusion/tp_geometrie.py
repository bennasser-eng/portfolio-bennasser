...
def BezierPlot(ax,xptcontrole,yptcontrole,color3):
    """ Display the whole B̩zier curve
    """
...
def BezierPlotOnePoint(ax,xptcontrole,yptcontrole,t0,color7 ,ms=8):
    """ Display value P(t0) of the B̩zier curve
    # on ajpute ce parametre ms par defaut ms=8 , pour donner plus
    de liberte dans le choix de ms (taille de marqueur)
    """
    degree = len(xptcontrole) - 1
    x0 = 0.
    y0 = 0.
...
def OneSubdivBezierCurve(xpt, ypt):
    """ One subdivision of the input control polygon according parameter 1/2
        Return the two subdivided control polygons 
            ...
            xpt2, ypt2
    """
    n = len(xpt) - 1 # degree of the B̩zier curve
    xp = np.zeros(2*n+1)
    yp = np.zeros(2*n+1)
    # Initialization
    for i in range(n+1):
...
def IntersectBox1D(I1, I2):
    ...
    if ( I1[1] < I2[0] ) or ( I1[0] > I2[1] ) :
        ...
        K = I1.copy()  
        #ici ,on prefere utiliser une copie de I1 
        #plutot que I1 elle meme 'reference' , 
        inter = 1
        if I1[0] < I2[0] :
            K[0] = I2[0]
        if I1[1] > I2[1] :
...
def BezierIntersection(ax,xp1,yp1,I1,xp2,yp2,I2,eps):
    ...
    """
    global ListRoots_u, ListRoots_v  #des variables globales , pour qu'on puisse conserver leurs valeurs 

    # Compute bounding boxes:
    box1 = MinMaxBoxCP(xp1, yp1)
    box2 = MinMaxBoxCP(xp2, yp2)

    # initial test : Check if the boxes intersect
    inter, _ = IntersectBox2D(box1, box2)
    if not inter:
        return          # No intersection, stop recursion



    #  Check if both intervals are smaller than eps
    len_I1 = I1[1] - I1[0]
    len_I2 = I2[1] - I2[0]
    
    if len_I1 < eps and len_I2 < eps:
        # Approximate parameters (midpoints of intervals)
        u0 = (I1[0] + I1[1]) / 2.0
        v0 = (I2[0] + I2[1]) / 2.0
        # Plot approximate points: blue for P(u0), red for Q(v0)
        BezierPlotOnePoint(ax, xp1, yp1 , u0, 'ob')     # Blue point
        BezierPlotOnePoint(ax, xp2, yp2, v0, 'or')      # Red point
        
        # Appliquer :Newton-Raphson for refinement:
        u, v = Newton2Var(xp1, yp1, I1, xp2, yp2, I2, NbOfNewtonSteps)
    
        # Check if solution is valid and within intervals
        if (0 <= u <= 1 and 0 <= v <= 1):    
            # on evite les duplications 
            duplicate = False
            for i in range(len(ListRoots_u)):
                if (abs(ListRoots_u[i] - u) < 1e-4 and 
                    abs(ListRoots_v[i] - v) < 1e-4):
                    duplicate = True
                    break    
            if not duplicate:
                ListRoots_u.append(u)
                ListRoots_v.append(v)
                # Plot refined points: blue cross for P(u), red cross for Q(v)
                BezierPlotOnePoint(ax, xp1, yp1, u, 'b+', ms=3)        # Blue cross : on prend ms=3 pour qu'on puisse voir le point vert de plus
                BezierPlotOnePoint(ax, xp2, yp2, v, 'r+', ms=3)        # Red cross  : meme remarque concernant ms=3
                
                # Plot intersection point as green : le centre du box :
                BezierPlotOnePoint(ax, xp1, yp1, u0, 'og',ms=10) # Green point , dans la premiere courbe 
                #BezierPlotOnePoint(ax, xp1, yp1, v0, 'og',ms=10) # Green point   , dzns la deuxieme courbe
                #on a le choix entre l'affichage de l'une ou l'autre
        return
        
 
    # tests and recursivity ...
    # Subdivide both curves at t = 0.5 : middle
    xpt1_1, ypt1_1, xpt1_2, ypt1_2 = OneSubdivBezierCurve(xp1, yp1)
    xpt2_1, ypt2_1, xpt2_2, ypt2_2 = OneSubdivBezierCurve(xp2, yp2)

    # Update parametric intervals for subdivided curves
    I1_mid = (I1[0] + I1[1]) / 2.0
    I2_mid = (I2[0] + I2[1]) / 2.0
    ...
    # Recursively process all four combinations of subdivided curves
    BezierIntersection(ax, xpt1_1, ypt1_1, I1_1, xpt2_1, ypt2_1, I2_1, eps)
    BezierIntersection(ax, xpt1_1, ypt1_1, I1_1, xpt2_2, ypt2_2, I2_2, eps)
    BezierIntersection(ax, xpt1_2, ypt1_2, I1_2, xpt2_1, ypt2_1, I2_1, eps)
    BezierIntersection(ax, xpt1_2, ypt1_2, I1_2, xpt2_2, ypt2_2, I2_2, eps)
...
def BezierDerivative1D(PC,t0):
    """ PC : list of scalar values (control points)
        Return value P'(t0) of the B̩zier function 
    """
    degree = len(PC) - 1
    PC = np.array(PC)
...
if __name__ == '__main__':
    ...
    if ListRoots_u == []:
        ...
        #on ajoute affichage des resultats:
        for i, (u, v) in enumerate(zip(ListRoots_u, ListRoots_v)):
            x_int = BezierValue1D(xp1, u)
            y_int = BezierValue1D(yp1, u)
            print(f"Intersection {i+1}: u={u:.6f}, v={v:.6f}, point=({x_int:.6f}, {y_int:.6f})")
